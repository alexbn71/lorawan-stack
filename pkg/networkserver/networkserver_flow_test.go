...
func handleClassAOTAAEU868FlowTest1_0_2(ctx context.Context, reg DeviceRegistry, tq DownlinkTaskQueue) {
	t := test.MustTFromContext(ctx)
	a := assertions.New(t)
...
func handleClassCOTAAUS915FlowTest1_0_3(ctx context.Context, reg DeviceRegistry, tq DownlinkTaskQueue) {
	t := test.MustTFromContext(ctx)
	a := assertions.New(t)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			for flow, handleFlowTest := range map[string]func(context.Context, DeviceRegistry, DownlinkTaskQueue){
				...
				"Class C/OTAA/US915/1.0.3": handleClassCOTAAUS915FlowTest1_0_3,
			} {
