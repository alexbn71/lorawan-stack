...
type MACSettingConfig struct {
	...
	DesiredADRAckLimit     *ttnpb.ADRAckLimitExponent `name:"desired-adr-ack-limit" description:"Desired ADR_ACK_LIMIT value Network Server should use if not configured in device's MAC settings"`
	DesiredADRAckDelay     *ttnpb.ADRAckDelayExponent `name:"desired-adr-ack-delay" description:"Desired ADR_ACK_DELAY value Network Server should use if not configured in device's MAC settings"`
